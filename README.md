...
|P0263|丑数|2019.12.30|描述：编写一个程序判断给定的数是否为丑数。丑数就是只包含质因数 2, 3, 5 的正整数.<br>思路：不断除以2/3/5，如果不能整除，则false，为1，则返回true.|掌握方法.|
|P0264|丑数II|2019.12.30|描述：编写一个程序，找出第 n 个丑数。丑数就是只包含质因数 2, 3, 5 的正整数。<br>思路：动态规划，使用三个指针，每个指针只有在dp[pos_x]*x是最小数的时候才会向右移动.也就是每个数都必须乘以2/3/5，然后对应指针才右移.|掌握该题目.|
|P0268|缺失数字|2019.12.29|描述：给定一个包含 0, 1, 2, ..., n 中 n 个数的序列，找出 0 .. n 中没有出现在序列中的那个数.<br>思路：方法1.高斯求和法；方法2.异或法；方法3.排序对比法.|掌握三种方法.|
...
|PO541|反转字符串II|2019.12.29|描述：给定一个字符串和一个整数 k，你需要对从字符串开头算起的每个 2k 个字符的前k个字符进行反转.<br>思路：计算每K个字符的下标left,right，然后falg为真，则翻转，否则不翻转，falg=!flag.|掌握方法.|
|P0557|反转字符串中的单词 III|2019.12.29|描述：给定一个字符串，你需要反转字符串中每个单词的字符顺序，同时仍保留空格和单词的初始顺序.<br>思路：找到空格位置，然后使用双指针法对该段单词进行首尾交换.|掌握方法.|
